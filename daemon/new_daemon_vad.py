#!/usr/bin/env python3
"""VAD-enhanced paDC daemon - only buffers audio when voice is detected"""
...
# ============================================================================
# VAD-ENHANCED AUDIO RECORDER
# ============================================================================

class VADAudioRecorder:
    """VAD-enhanced audio recorder - only buffers audio when voice is detected"""
    ...
    def __init__(self, sample_rate: int = 16000, channels: int = 1, buffer_seconds: float = 60.0):
        ...
        # VAD-filtered buffer (only contains speech)
        ...
        # Buffer for accumulating small chunks before VAD processing
        # Silero VAD requires minimum 512 samples (30ms at 16kHz)
        self.vad_chunk_buffer = []
        self.min_vad_samples = 512

    def _initialize_vad(self):
        """Initialize Silero VAD model"""
        try:
            print(f"[{time.strftime('%H:%M:%S')}] Loading Silero VAD model...")
            ...
            print(f"[{time.strftime('%H:%M:%S')}] Silero VAD model loaded successfully")
        except Exception as e:
            print(f"ERROR: Failed to load Silero VAD model: {e}")
            sys.exit(1)

    def _is_speech(self, audio_chunk: np.ndarray) -> tuple[bool, float]:
        """
        ...
        try:
            ...
            # Our audio is already float32 from sounddevice
            ...
            # Threshold: 0.5 is default, adjust if needed
            is_speech = speech_prob > 0.5

            return is_speech, speech_prob
        except Exception as e:
            print(f"VAD error: {e}")
            # On error, assume it's speech to avoid losing audio
            return True, 1.0
    ...
    def start(self, play_chime=True):
        ...
        self.vad_chunk_buffer = []  # Clear VAD chunk buffer
            ...
            while self.recording:
                try:
                    ...
                    if self.chunks_per_second is None:
                        ...
                        print(f"[{time.strftime('%H:%M:%S')}] VAD buffer initialized: {max_chunks} chunks max ({self.buffer_seconds}s)")
                        print(f"[{time.strftime('%H:%M:%S')}] Chunk size: {frames_per_chunk} samples, min VAD: {self.min_vad_samples} samples")

                    # Accumulate chunks until we have enough for VAD
                    self.vad_chunk_buffer.append(data)
                    accumulated_samples = sum(len(chunk) for chunk in self.vad_chunk_buffer)

                    if accumulated_samples >= self.min_vad_samples:
                        # Concatenate accumulated chunks
                        combined = np.concatenate(self.vad_chunk_buffer, axis=0)

                        # Extract exactly 512 samples for VAD
                        vad_chunk = combined[:self.min_vad_samples]

                        # Keep any remaining samples for next iteration
                        if len(combined) > self.min_vad_samples:
                            self.vad_chunk_buffer = [combined[self.min_vad_samples:]]
                        else:
                            self.vad_chunk_buffer = []

                        # Apply VAD filtering - only add to buffer if speech detected
                        is_speech, confidence = self._is_speech(vad_chunk)

                        self.total_chunks += 1

                        if is_speech:
                            self.audio_data.append(vad_chunk)
    ...
    def get_buffer_snapshot(self) -> tuple[np.ndarray, dict]:
        """Get a copy of the current buffer without stopping recording

        Returns:
            tuple: (audio_buffer, vad_stats)
        """
        ...
        # Reset statistics for next recording
        self.total_chunks = 0
        self.speech_chunks = 0
        self.silence_chunks = 0

        return np.concatenate(buffer_copy, axis=0), vad_stats
    ...
    def stop(self) -> tuple[np.ndarray, dict]:
        ...
        return np.concatenate(self.audio_data, axis=0), vad_stats
...
class PaDCDaemon:
    def __init__(self):
        ...
        self.recorder = VADAudioRecorder(buffer_seconds=buffer_seconds)
        self.whisper = None
        self.running = True
    ...
    def start_recording(self):
        ...
        print(f"[{time.strftime('%H:%M:%S')}] Recording started (VAD-filtered)... ", end="", flush=True)
    ...
    def stop_recording(self):
        ...
        audio_buffer, vad_stats = self.recorder.get_buffer_snapshot()
        ...
        threading.Thread(
            ...
            args=(audio_buffer, vad_stats, processing_start_time, self.recording_mode),
            daemon=True
        ).start()
    ...
    def _transcribe(self, audio_buffer, vad_stats, processing_start_time, recording_mode):
        ...
        try:
            ...
            if text:
                ...
                # Display consolidated transcription info with VAD stats
                ...
                # Add VAD statistics
                if vad_stats:
                    log_lines.append(f"│ VAD: {vad_stats['speech_chunks']}/{vad_stats['total_chunks']} chunks speech ({vad_stats['speech_ratio']*100:.1f}%)")
                    log_lines.append(f"│      {vad_stats['silence_chunks']} chunks filtered")
                ...
                if info.get('has_speech') is False:
                    log_msg = [
                        f"\n[{timestamp}]",
                        f"┌─ Transcription ({total_time:.2f}s)",
                        f"│ Buffer: {info.get('buffer_duration', 0):.2f}s"
                    ]
                    if vad_stats:
                        log_msg.append(f"│ VAD: {vad_stats['speech_chunks']}/{vad_stats['total_chunks']} chunks speech ({vad_stats['speech_ratio']*100:.1f}%)")
                    log_msg.append(f"└─ No speech detected")
    ...
    def run(self):
        ...
        print(
            f"[{time.strftime('%H:%M:%S')}] paDC VAD daemon started (PID: {os.getpid()})",
            flush=True,
        )
        print(
